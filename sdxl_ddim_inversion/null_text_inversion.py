...
class NullInversion:
    ...
    def _get_text_embeddings(self, prompt: str, tokenizer, text_encoder, device):
        # Add self parameter and change from static to instance method
    ...
    def _encode_text_sdxl(self, prompt: str) -> tuple[dict[str, T], T]:
        ...
        prompt_embeds, pooled_prompt_embeds = self._get_text_embeddings(
            prompt, 
            self.model.tokenizer, 
            self.model.text_encoder, 
            device
        )
        prompt_embeds_2, pooled_prompt_embeds2 = self._get_text_embeddings(
            prompt, 
            self.model.tokenizer_2, 
        ...
        # Make sure add_time_ids has correct batch dimension (1)
        add_time_ids = add_time_ids.unsqueeze(0)
        
        added_cond_kwargs = {
            "text_embeds": pooled_prompt_embeds2.unsqueeze(0),  # Add batch dimension
    ...
    def _encode_text_sdxl_with_negative(self, prompt: str) -> tuple[dict[str, T], T]:
        ...
        # Concatenate and properly shape the conditional inputs
        added_cond_kwargs = {
            "text_embeds": torch.cat((
                added_cond_kwargs_uncond["text_embeds"], 
                added_cond_kwargs["text_embeds"]
            ), dim=0),
            "time_ids": torch.cat((
                added_cond_kwargs_uncond["time_ids"], 
    ...
    def get_noise_pred_only(self, latent: T, t: T, context: T, added_cond_kwargs: dict[str, T]):
        ...
        context = context.to(dtype=torch.float16)
        
        # Make sure added_cond_kwargs tensors have the correct dimensions
        for key in added_cond_kwargs:
            if isinstance(added_cond_kwargs[key], torch.Tensor):
                # Get the current shape
                current_shape = added_cond_kwargs[key].shape
                # Create repeat tuple based on tensor dimensions
                repeat_shape = (2,) + tuple(1 for _ in range(len(current_shape)-1))
                # Repeat the tensor properly
                added_cond_kwargs[key] = added_cond_kwargs[key].repeat(*repeat_shape)
                added_cond_kwargs[key] = added_cond_kwargs[key].to(dtype=torch.float16)
    ...
    def null_optimization(self, latents, num_inner_steps, epsilon):
        ...
        for i in range(self.ddim_steps):
            ...
            uncond_embeddings = uncond_embeddings.to(dtype=torch.float16)  # Ensure float16
            uncond_embeddings.requires_grad = True
            optimizer = Adam([uncond_embeddings], lr=1e-2 * (1. - i / 100.))
            latent_prev = latents[len(latents) - i - 2]
            t = self.model.scheduler.timesteps[i].to(dtype=torch.float16)  # Ensure float16

            for j in range(num_inner_steps):
                noise_pred_uncond, noise_pred_cond = self.get_noise_pred_only(
                    latent_cur.to(dtype=torch.float16), 
                    t, 
                    context.to(dtype=torch.float16), 
                    added_cond_kwargs
                )
    ...
    def invert(self, image_path: str, offsets=(0,0,0,0), num_inner_steps=10, early_stop_epsilon=1e-5, verbose=False):
        ...
        # Fixed: Pass all required arguments to ddim_inversion
        ddim_latents = DDIM_inversion.ddim_inversion(
            model=self.model,  # Pass the SDXL pipeline
            x0=image_gt,      # Pass the loaded image
            prompt=self.prompt,  # Pass the prompt
            num_inference_steps=self.ddim_steps,  # Pass number of steps
            guidance_scale=self.guidance_scale    # Pass guidance scale
        )
        if verbose:
